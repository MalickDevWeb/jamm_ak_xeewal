  ...
  <div *ngIf="visible" class="fixed inset-0 z-[99999] flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-3xl shadow-2xl max-w-md w-full p-8 animate-fade-in-up">
      ...
      <div class="flex gap-3">
        <button (click)="onCancel()" class="flex-1 px-6 py-3 bg-gray-100 text-gray-700 rounded-xl text-sm font-bold hover:bg-gray-200 transition-all">Annuler</button>
        <button (click)="onConfirm()" class="flex-1 px-6 py-3 bg-gradient-to-r from-red-600 to-red-700 text-white rounded-xl text-sm font-bold shadow-lg hover:shadow-xl hover:scale-105 transition-all">Supprimer</button>
      </div>
    </div>
  </div>
  `,
