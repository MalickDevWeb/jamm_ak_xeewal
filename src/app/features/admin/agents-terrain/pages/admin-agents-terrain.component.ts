    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p class="text-sm text-gray-500 mt-1">Gérez les agents qui inscrivent les citoyens sur le terrain.</p>
        </div>
...
export class AdminAgentsTerrainComponent implements OnInit {
  ...
  loadAgents() {
    this.isLoading = true;
    this.adminData.getAgentsTerrain().subscribe({
      next: (res: any) => {
  ...
  onCreateAgent(event: Event) {
    ...
    this.adminData.createEntity('agents-terrain', this.newAgent).subscribe({
      next: (res: any) => {
        this.isCreating = false;
        if (res.success) {
          this.createSuccess = true;
          this.agents.unshift(res.data);
          // Keep newAgent data for WhatsApp link until modal closes
        } else {
  ...
  toggleAgent(agent: AgentTerrain) {
    this.adminData.updateEntity('agents-terrain', agent.id, { actif: !agent.actif }).subscribe({
      next: (res: any) => {
        if (res.success) {
  ...
  deleteAgent(agent: AgentTerrain) {
    if (!confirm(`Supprimer l'agent ${agent.prenom} ${agent.nom} ?`)) return;
    this.adminData.deleteEntity('agents-terrain', agent.id).subscribe({
      next: () => {
