...
@Component({
  ...
  <div class="animate-fade-in-up">
    <div class="flex items-center justify-between mb-8">
      <div>
        <h2 class="text-2xl font-black text-white">Sondages</h2>
        <p class="text-sm text-gray-400 mt-1">{{ total }} sondages créés pour consulter les citoyens.</p>
      </div>
    ...
    <div *ngIf="!isLoading" class="space-y-6">
      <div *ngFor="let s of sondages; trackBy: trackById" class="bg-white/5 border border-white/10 rounded-2xl shadow-sm border border-white/10 p-6 hover:shadow-md transition-shadow">
        <div class="flex items-start justify-between gap-4 mb-5">
          <div class="flex-1">
            <div class="flex items-center gap-3 mb-2">
              <span [class]="s.statut === 'ACTIF' ? 'bg-brand-green/10 text-brand-green' : 'bg-white/10 text-gray-400'"
            ...
            <h3 class="text-lg font-bold text-white">{{ s.question }}</h3>
          </div>
        </div>
        ...
        <div class="space-y-3">
          <div *ngFor="let option of s.options; trackBy: trackByOptionId" class="group">
            <div class="flex items-center justify-between mb-1">
              <span class="text-sm font-medium text-gray-200">{{ option.texte }}</span>
              <div class="flex items-center gap-2">
                <span class="text-sm font-bold text-white">{{ option.votes }}</span>
        ...
        <div class="flex items-center justify-between mt-5 pt-4 border-t border-gray-50">
          ...
          <div class="flex gap-2">
            <button (click)="action('Supprimer', s.id)" class="px-3 py-1.5 text-xs font-bold text-red-600 bg-red-50 hover:bg-red-500/10 rounded-lg transition-colors"><i class="fa-solid fa-trash"></i></button>
            <button (click)="action('Clôturer', s.id)" *ngIf="s.statut === 'ACTIF'" class="px-3 py-1.5 text-xs font-bold text-gray-300 bg-white/10 hover:bg-white/20 rounded-lg transition-colors">Clôturer</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div *ngIf="showModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div class="bg-white/5 border border-white/10 rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden animate-fade-in-up">
        <div class="px-6 py-4 border-b border-white/10 flex justify-between items-center bg-black/20">
          <h3 class="font-black text-white text-lg">Créer un sondage</h3>
        ...
        <div class="p-6">
          <div class="mb-4">
            <label class="block text-sm font-bold text-gray-200 mb-1">Question du sondage</label>
            <input type="text" [(ngModel)]="formData.question" class="w-full px-4 py-2.5 rounded-xl border border-white/20 focus:border-brand-green focus:ring-2 focus:ring-brand-green/20 transition-all outline-none" placeholder="Ex: Quel est le projet prioritaire ?">
          </div>
          <div class="mb-4">
            <label class="block text-sm font-bold text-gray-200 mb-1">Options (séparées par une virgule)</label>
            <input type="text" [(ngModel)]="formData.optionsStr" class="w-full px-4 py-2.5 rounded-xl border border-white/20 focus:border-brand-green focus:ring-2 focus:ring-brand-green/20 transition-all outline-none" placeholder="Ex: Hôpital, Ecole, Route">
          </div>
